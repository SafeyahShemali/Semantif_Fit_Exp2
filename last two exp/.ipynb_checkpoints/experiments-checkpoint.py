    ...
    for i in range(len(dataset)): 
        ...
        for sentence in sentences:
            ...
            if is_semantic:  
                ...
                '''Retrieve the data from previous exp'''
                print(f'Result/lemma_tuple_reasoning_categorical_{dataset_name}_{model_name_only}.csv')
...
def exp_reasoning_gen_sentences_other(filename: str, model: Model, model_name: str):
    ...
    for i in range(len(dataset)): 
        ...
        '''TRACING'''
        print('New Record : ', predicate, argument, roleType)
        print('New COnversation : ', model.conversation , '\n')
        ...
        '''Retrive the reasons'''
        # the reasons are saved on the 4th position of the record
        reasons = data.retrive_reasons(reason_filename, predicate, argument, roleType)
